...
from examples.hancock.clinical import ClinicalDataError, load_clinical_records, survival_table
...
def test_an_unset_endpoint_is_refused() -> None:
    """The library carries no column names, and empty ones would censor everybody."""
    from examples.hancock.clinical import endpoint_from_config
    from kalecancer.config import get_cfg_defaults

    with pytest.raises(ClinicalDataError, match="endpoint is unset"):
        endpoint_from_config(get_cfg_defaults())
