...
from examples.hancock.cohort import COHORT_COLUMNS, build_cohort
...
def test_feature_validation_can_be_skipped(feature_root: Path, clinical_path: Path) -> None:
    ...
    cohort = build_cohort(
        feature_root, clinical_path, endpoint=OS_ENDPOINT, expected_dim=FEATURE_DIM, validate_features=False
    )
