"""Tests for ``kalecancer.evaluate.cross_validation``."""
...
def test_index_inputs_handles_variable_length_bag_lists() -> None:
    ...
    indexed_tabular, indexed_bags = indexed["tabular"], indexed["bags"]
    assert isinstance(indexed_tabular, torch.Tensor)
    assert isinstance(indexed_bags, list)

    assert torch.equal(indexed_tabular, tabular[idx])
    assert len(indexed_bags) == len(idx)
    for out_bag, original_i in zip(indexed_bags, idx.tolist(), strict=True):
