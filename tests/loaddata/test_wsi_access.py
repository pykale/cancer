...
The slide-naming pattern is an argument rather than a default, so these declare one
the way a dataset would.
"""
...
#: A stand-in for whatever a real collection uses; the point is that it is given.
PATTERN = re.compile(r"^.+?_(?P<patient_id>\d+)(?:_[a-z])?$")
...
def test_parse_patient_id_handles_multi_slide_suffix(slide_id: str, expected: str) -> None:
    assert parse_patient_id(slide_id, PATTERN) == expected
...
def test_parse_patient_id_rejects_unparseable_name() -> None:
    with pytest.raises(SlideIdentifierError, match="cannot parse a patient id"):
        parse_patient_id("slide_without_number", PATTERN)


def test_slide_table_walks_nested_directories(feature_root: Path) -> None:
    slides = slide_table(feature_root, PATTERN)
...
def test_slide_table_is_empty_for_a_directory_without_features(tmp_path: Path) -> None:
    ...
    slides = slide_table(empty, PATTERN)
...
def test_slide_table_requires_existing_root(tmp_path: Path) -> None:
    with pytest.raises(FileNotFoundError):
        slide_table(tmp_path / "absent", PATTERN)
