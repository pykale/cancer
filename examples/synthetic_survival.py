...
Run: python examples/synthetic_survival.py
"""
...
from kalecancer.evaluate.cross_validation import fit_survival_model
...
def main() -> None:
    ...
    history = fit_survival_model(
        model,
        data.embeddings[train_idx_t],
        data.times[train_idx_t],
        data.events[train_idx_t],
        val_inputs=data.embeddings[test_idx_t],
        val_times=data.times[test_idx_t],
        val_events=data.events[test_idx_t],
        max_epochs=200,
        lr=1e-2,
        patience=20,
        seed=0,
    ...
    print(
        f"train loss: {history['train_loss'][0]:.4f} -> {history['train_loss'][-1]:.4f} ({history['epochs_run']} epochs)"
    )
    ...
    auc_per_time, mean_auc = time_dependent_auc(
        train_times, train_events, test_times, test_events, test_log_hazard, EVAL_DAYS
    )
    ...
    cv = cross_validate_survival(
        lambda: SurvivalModel(n_features), data.embeddings, data.times, data.events, **CV_KWARGS
    )
    for fold in cv["folds"]:
        print(
            f"  fold {fold['fold']}: C-index={fold['c_index']:.3f}  IBS={fold['integrated_brier']:.4f}  log-rank p={fold['log_rank_p_value']:.2e}"
        )
    agg = cv["aggregated"]
    ...
    for row in table:
        print(
            f"  {row['model']:>12}: C-index={row['c_index']:.3f} +/- {row['c_index_std']:.3f}  IBS={row['integrated_brier']:.4f}  td-AUC={row['td_auc_mean']:.3f}"
        )
