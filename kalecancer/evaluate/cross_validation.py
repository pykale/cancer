"""Cross-validation and resampled confidence intervals for survival models.

Two ways of asking how far a number would travel: :func:`cross_validate_survival`
refits on every fold, so the spread is over *models*, while :func:`bootstrap_ci`
resamples one set of predictions, so the spread is over *patients*. They answer
...
For each fold, the baseline hazard (:func:`~kalecancer.evaluate.survival_metrics.breslow_baseline_hazard`)
...
from kalecancer.loaddata.splitting import CrossValidation
...
ModelInputs: TypeAlias = torch.Tensor | Mapping[str, "torch.Tensor | Sequence[torch.Tensor]"]


def _call_model(model: nn.Module, inputs: ModelInputs) -> torch.Tensor:
...
def fit_survival_model(
    ...
    events: torch.Tensor,
    *,
    val_inputs: ModelInputs | None = None,
    ...
    """
    # Bundled rather than checked as three separate flags: it is the triple that is
    # either present or absent, and binding it once lets every later use see that.
    validation: tuple[ModelInputs, torch.Tensor, torch.Tensor] | None = None
    if val_inputs is not None and val_times is not None and val_events is not None:
        validation = (val_inputs, val_times, val_events)
    elif val_inputs is not None or val_times is not None or val_events is not None:
    ...
    for epoch in range(max_epochs):
        ...
        if validation is not None:
            fold_inputs, fold_times, fold_events = validation
            model.eval()
            with torch.no_grad():
                val_log_hazard = _call_model(model, fold_inputs)
                val_loss_value = neg_partial_log_likelihood(val_log_hazard, fold_times, fold_events).item()
        ...
        if validation is not None and epochs_without_improvement >= patience:
            break
...
def patient_stratified_splits(
    ...
    (:func:`~kalecancer.model.predict.losses.neg_partial_log_likelihood` raises),
    ...
    splitter = CrossValidation(n_splits=n_splits, val_size=0.0, stratify_by=["event"], random_state=seed)
    folds = list(splitter.split(pd.DataFrame({"event": events})))
...
def _index_inputs(inputs: ModelInputs, idx: torch.Tensor) -> ModelInputs:
...
def compare_models(
    ...
    inputs_per_model: Mapping[str, ModelInputs],
    times: torch.Tensor,
    events: torch.Tensor,
    *,
    eval_years: tuple[float, ...] = (1, 3, 5),
