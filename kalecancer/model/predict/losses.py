"""Training objectives, and the helpers that keep their conventions straight.

Every loss a head is trained with lives here, so the objective and the head that
emits scores for it are found together.

Two shapes of loss. The plain ones -- :func:`neg_partial_log_likelihood`,
:func:`binary_cross_entropy` -- score a tensor of predictions. The ``multimodal_``
pair wrap them for a :class:`~kalecancer.model.embed.MultimodalOutput`, adding an
optional per-modality term that keeps every embedder learning when one modality
dominates.

The event convention is fixed throughout: ``1`` observed, ``0`` censored.
:func:`as_event_mask` is the single place that converts to the boolean form the Cox
loss needs, so a float/bool mix-up cannot silently invert a model's predictions.
"""
...
def binary_cross_entropy(
    prediction: torch.Tensor, targets: torch.Tensor, pos_weight: torch.Tensor | None = None
) -> torch.Tensor:
    """Cross-entropy on logits, for a binary endpoint.

    Logits rather than probabilities: the fused sigmoid-and-log form is the
    numerically stable one.

    Args:
        prediction: ``(B,)`` or ``(B, 1)`` logits.
        targets: ``(B,)`` targets, 1 positive and 0 negative.
        pos_weight: Weight on the positive class, for an imbalanced endpoint.
    """
...
def multimodal_bce_loss(
    ...
    loss = binary_cross_entropy(output.prediction, targets, pos_weight)
    if auxiliary_weight <= 0 or not output.modality_predictions:
        return loss

    auxiliary = torch.stack(
        [binary_cross_entropy(prediction, targets, pos_weight) for prediction in output.modality_predictions.values()]
    )
    return loss + auxiliary_weight * auxiliary.mean()
